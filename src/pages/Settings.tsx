...
export default function Settings() {
  ...
  const updateAdminMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: AdminFormPayload }) =>
      updateAdminApi(id, { username: data.username, email: data.email, password: data.password || undefined }),
    onSuccess: () => {
  ...
  const openEditAdmin = (admin: AdminListItem) => {
    setEditingAdmin({ id: admin.id, username: admin.username, email: admin.email });
    setAdminModalError(null);
    setAdminModalOpen(true);
  };
  ...
  const adminModalLoading = createAdminMutation.isPending || updateAdminMutation.isPending;
    ...
    <div className="mx-auto max-w-4xl px-4 py-8 space-y-10">
      ...
      <section className="rounded-xl border border-[#3A1A22] bg-[#241217]/80 shadow-lg overflow-hidden">
        <div className="px-6 py-4 border-b border-[#3A1A22] bg-[#1B0F12]/50 flex items-center gap-2">
          ...
          <h2 className="text-lg font-medium text-[#F5DEB3]">Change Password</h2>
        </div>
            ...
            <table className="w-full border-collapse">
              ...
              <tbody className="divide-y divide-[#3A1A22]">
                {admins.map((admin) => (
                  ...
                  >
                    <td className="px-6 py-4 font-medium text-[#F5DEB3]">{admin.username}</td>
                    ...
                    <td className="px-6 py-4 text-right flex items-center justify-end gap-2">
                      ...
                      {deleteConfirmId === admin.id ? (
                        <span className="flex items-center gap-2">
                          <span className="text-xs text-[#F5DEB3]/70">Remove?</span>
                          <button
      ...
      <AdminModal
        key={adminModalOpen ? (editingAdmin ? `edit-${editingAdmin.id}` : "add") : "closed"}
        open={adminModalOpen}
        mode={editingAdmin ? "edit" : "add"}
        initialValues={editingAdmin ? { username: editingAdmin.username, email: editingAdmin.email } : undefined}
