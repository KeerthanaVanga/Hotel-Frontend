...
export default function OffersPage() {
  ...
  const [confirm, setConfirm] = useState<{ open: boolean; offerId?: number; label?: string }>({
    open: false,
  });
  ...
  if (isError) {
    return (
      <section className="space-y-6">
        ...
        <p className="text-red-400">{(error as Error)?.message || "Failed to load offers"}</p>
      </section>
    );
  }
      ...
      {data.length === 0 ? (
        ...
        <>
          <OffersTable
            ...
            onDelete={(id, label) => setConfirm({ open: true, offerId: Number(id), label })}
          />
