...
export default function PaymentStatusChart({
  ...
  const chartData = data.map((d) => {
    const normalizedStatus = d.status.toLowerCase() === "success" ? "paid" : d.status.toLowerCase();

    const value =
      normalizedStatus === "paid"
        ? d.paidAmount
        : d.pendingAmount; // pending + partial show outstanding money
...
/>
      ...
      <div className="mt-4 space-y-2 text-sm">
        {data.map((d) => {
          const status = d.status.toLowerCase() === "success" ? "paid" : d.status.toLowerCase();
          return (
