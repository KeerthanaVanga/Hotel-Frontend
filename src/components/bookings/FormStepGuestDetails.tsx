...
import type { CreateBookingFormValues, GuestType } from "../../types/CreateBookingForm";
      ...
      {values.guest_type === "existing" && (
        <div ref={dropdownRef} className="relative">
          ...
          {errors.selected_user_id && touched.selected_user_id && (
            <p className="mt-1 text-xs text-red-400">{errors.selected_user_id}</p>
          )}
          {dropdownOpen && (
            <ul
              ...
              {filteredUsers.length === 0 ? (
                ...
                filteredUsers.map((user) => (
                  <li
                    key={user.user_id}
                    role="option"
                    aria-selected={values.selected_user_id === String(user.user_id)}
