    ...
    <>
              ...
              return (
                <tr key={o.id} className="hover:bg-white/5">
                  <td className="px-4 py-3">
                    <div className="font-medium text-[#F5DEB3]">{o.roomName}</div>
                    <div className="text-xs text-[#F5DEB3]/60">{o.roomType}</div>
                  </td>
                  <td className="px-4 py-3 text-center">{o.title}</td>
                  <td className="px-4 py-3 text-center">₹{o.originalPrice}</td>
                  <td className="px-4 py-3 text-center">{o.discountPercent}%</td>
                  <td className="px-4 py-3 text-center">₹{o.offerPrice}</td>
                  <td className="px-4 py-3 text-center">
                    <span
                      className={`rounded-full px-2.5 py-1 text-xs font-medium ${
                        o.active ? "bg-emerald-500/15 text-emerald-400" : "bg-red-500/15 text-red-400"
                      }`}
                    >
      ...
      <div className="space-y-4 md:hidden">
        {offers.map((o) => {
          const label = `${o.roomName}`;
          return (
            <div key={o.id} className="rounded-xl border border-[#3A1A22] bg-[#241217] p-4 space-y-2">
