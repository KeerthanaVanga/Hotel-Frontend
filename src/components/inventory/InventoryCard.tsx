...
export default function InventoryCard({
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4 rounded-xl border border-[#3A1A22] bg-[#241217] p-4 hover:border-[#D4AF37] transition">
      <img src={hotel.image} className="h-40 w-full md:w-56 rounded-lg object-cover" />
      ...
      <div className="flex-1 space-y-2 min-w-0">
        ...
        <div className="flex flex-wrap gap-2 max-w-full">
          {hotel.amenities.slice(0, 8).map((a) => (
            <span key={a} className="rounded-full bg-[#3A1A22] px-2 py-1 text-xs text-[#F5DEB3]/80">
              {a}
            </span>
          ))}
      ...
      <div className="flex flex-col justify-between items-end">
        <span className="text-xl font-semibold text-[#D4AF37]">₹{hotel.price}</span>
        <button
