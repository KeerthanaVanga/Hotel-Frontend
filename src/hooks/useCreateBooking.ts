...
export function useCreateBooking(showToast: (type: "success" | "error", message: string) => void) {
  ...
  function getStepTouchedFields(step: number): Partial<Record<keyof CreateBookingFormValues, boolean>> {
    switch (step) {
      case 1:
  ...
  async function handleNext(
    ...
    setTouched: (touched: Partial<Record<keyof CreateBookingFormValues, boolean>>) => void
  ) {
    setAvailabilityError(null);
    setTouched(getStepTouchedFields(step));
    const errs = await validateForm();
    const fields = Object.keys(currentSchema.fields) as (keyof CreateBookingFormValues)[];
    ...
    if (step === 1) {
      ...
      if (result && !result.available) {
        setAvailabilityError(
          result.message || "This room is not available for the selected dates."
        );
        return;
      }
      if (result?.available) {
